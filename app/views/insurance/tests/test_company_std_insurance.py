...
class CompanyStdInsuranceTestCase(TestCase, ViewTestBase):
    # your fixture files here
    fixtures = ['std_insurance', '23_auth_user',
                '49_period_definition', '10_company']

    def test_get_company_std_insurance_success(self):
        response = self.client.get(reverse('company_std_insurance_plan_api',
    ...
    def test_get_company_std_insurance_invalid_company(self):
        response = self.client.get(reverse('company_std_insurance_plan_api',
    ...
    def test_post_company_std_insurance_success(self):
        std_data = {'company': self.normalize_key(3),
                    'std_insurance_plan': self.normalize_key(1),
                    'elimination_period_in_days': 234, 
                    'duration': 90, 
                    'percentage_of_salary': '80.00', 
                    'max_benefit_weekly': '1122.00', 
                    'rate': '0.220000', 
                    'age_based_rates': [], 
                    'employer_contribution_percentage': '22.00', 
                    'paid_by': None}
        response = self.client.post(reverse('company_std_insurance_plan_api',
        ...
        self.assertEqual(result['elimination_period_in_days'], 234)
        self.assertEqual(result['max_benefit_weekly'], '1122.00')
        self.assertEqual(result['percentage_of_salary'], '80.00')
        self.assertEqual(result['employer_contribution_percentage'], '22.00')

    def test_post_company_std_insurance_age_based_rates_success(self):
        std_data = {'company': self.normalize_key(3),
                    'std_insurance_plan': self.normalize_key(1),
                    'elimination_period_in_days': 234, 
                    'duration': 90, 
