...
class CompanyBenefitAvailabilityTestCase(TestCase, ViewTestBase):
    ...
    def test_get_company_benefit_summary(self):
        response = self.client.get(reverse('company_benefit_availability_api', kwargs={'company_id': self.normalize_key(1)}))
        ...
        self.assertIn('medical', summary)
        self.assertIn('dental', summary)
        self.assertIn('vision', summary)
    ...
    def test_get_company_benefit_summary_no_benefit(self):
        response = self.client.get(reverse('company_benefit_availability_api', kwargs={'company_id': self.normalize_key(4)}))
        self.assertIsNotNone(response)
        self.assertEqual(response.status_code, 200)
        self.assertNotEqual(response.content, '')
        ...
        self.assertEqual(len(summary['medical']), 0)
        self.assertEqual(len(summary['dental']), 0)
        self.assertEqual(len(summary['vision']), 0)
    ...
    def test_get_company_benefit_summary_not_existed(self):
        response = self.client.get(reverse('company_benefit_availability_api', kwargs={'company_id': self.normalize_key(100)}))
        self.assertIsNotNone(response)
