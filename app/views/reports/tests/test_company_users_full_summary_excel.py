...
class CompanyUsersFullSummaryExcelTestCase(TestCase, ViewTestBase):
    ...
    def test_get_full_summary_excel_success(self):
        if self.client.login(username='user2@benefitmy.com', password='foobar'):
            response = self.client.get(reverse('company_full_summary_excel_api',
    ...
    def test_get_full_summary_excel_non_exist(self):
        if self.client.login(username='user2@benefitmy.com', password='foobar'):
            response = self.client.get(reverse('company_full_summary_excel_api',
