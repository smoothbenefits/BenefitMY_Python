...
class ConnectPayrollEmployeeFrontPageCsvTestCase(TestCase, ViewTestBase):
    ...
    def test_get_connect_payroll_employee_front_page_csv_success(self):
        if self.client.login(username='user2@benefitmy.com', password='foobar'):
            response = self.client.get(reverse('company_connect_payroll_employee_frontpage_csv_api',
                                               kwargs={'company_id': self.normalize_key(2)}))
