    ...
    def __init__(self, person_model, company_id):
        ...
        if (self._employee_profile_model):
            ...
            if (self._compensation_service):
                ...
                projected_hours = self._get_projected_hours_per_pay_cycle(
                    current_compensation,
                    self._employee_profile_model.company.pay_period_definition
                )
                if (projected_hours):
                    self.projected_hours_per_pay_cycle = projected_hours
                ...
                if (self.pay_type == PAY_TYPE_HOURLY):
                    ...
                    self.current_pay_period_salary = self._get_pay_period_salary(
                        self._compensation_service.get_current_annual_salary(),
                        self._employee_profile_model.company.pay_period_definition
                    )
